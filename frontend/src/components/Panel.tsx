...
const TRACE_CAPTION =
  "line above 0 mean this lens favored the word more than baseline, below 0 favored less. a thicker line means the lens's whole set of likely next words differed more from baseline's."

const TRACE_DEEP_EXPLANATION =
  "At each word, the model isn't just picking one word — it's weighing many possible next words at once, each with its own odds. The vertical position compares one thing: how likely this lens was to pick the exact word that ended up here, versus how likely the plain baseline model was to pick that same word. Above zero, this lens favored it more; below, less. Thickness measures something else entirely: not this one word, but how different the lens's whole weighing of options was from baseline's at that moment — every word it was considering and how strongly. A thick ribbon sitting near zero means the lens was seriously considering a different set of words than baseline, and still happened to land on the same one. A thin ribbon far from zero means the lens's options looked a lot like baseline's, but it pushed hard for this specific word anyway."

const MIXED_COLOR_DEEP_EXPLANATION =
  "Each word here is tinted by whichever lens's predictions actually drove the model's choice at that step — the same color that lens uses in the rail, on its map, and in its own trace below. A run of one color means that lens was steering the answer for that stretch; a change in color means a different lens took over. The ribbon in the trace is colored the same way, so you can see exactly which color produced which part of the shape."
